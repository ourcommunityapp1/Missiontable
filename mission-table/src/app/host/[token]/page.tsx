...
import { getHostByToken, getMembersForGroup, getKitsForGroup } from '@/lib/queries';
...
export default async function HostDashboardPage({
  ...
  const members = group ? await getMembersForGroup(group.id) : { accepted: [], pending: [] };
  const kits = group ? await getKitsForGroup(group.id) : [];
                  ...
                  {members.pending.map((m) => (
                    <div key={m.membershipId} className="flex items-start justify-between gap-4 border-t-2 border-black py-4 last:border-b-2">
                      ...
                      <form
                        action={async () => {
                          'use server';
                          if (group) await approveMember(m.membershipId, group.id);
                        }}
                      >
                        <button
                          type="submit"
